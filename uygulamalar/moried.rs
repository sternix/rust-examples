// https://github.com/yuttie/moried
...
async fn main() {
    ...
    let api = {
        ...
        assert!(
            ...
            "MORIED_ROOT_PATH must start with '/'"
        );
        assert!(
            root_path.ends_with('/'),
            "MORIED_ROOT_PATH must end with '/'"
        );
    ...
    let addr = {
        let mut addrs_iter = env::var("MORIED_LISTEN")
            .unwrap()
            .to_socket_addrs()
            .unwrap();
...
mod filters {
    ...
    pub fn notes(
        ...
        let api = auth().and(closed).or(open);
        ...
        if let Some(root_path) = root_path {
            ...
            api.recover(handlers::rejection).boxed()
        }
    }
...
mod handlers {
    use super::models::{Cached, Claims, ListEntry, Login, NoteSave, State, Unauthorized};
    ...
    pub async fn login(login: Login) -> Result<Box<dyn warp::Reply>, warp::reject::Rejection> {
        ...
        let matches = user_name == login.user
            && argon2::verify_encoded(&user_hash, login.password.as_ref()).unwrap();
    ...
    pub async fn load_note(
        ...
        let found = {
            ...
            index
                .iter()
                .find(|entry| std::str::from_utf8(&entry.path).unwrap() == path.as_str())
        };
        if let Some(entry) = found {
            let found = {
                let repo = state.repo.lock().await;
                repo.find_blob(entry.id)
                    .map(|blob| Vec::from(blob.content()))
            };
            match found {
                Ok(content) => {
        ...
        match note_save {
            ...
            NoteSave::Rename { from } => {
                let found = {
                    ...
                    index
                        .iter()
                        .find(|entry| std::str::from_utf8(&entry.path).unwrap() == from)
                };
...
mod models {
    ...
    #[derive(Debug, Deserialize, Serialize, Clone)]
    pub enum NoteSave {
        Save { content: String, message: String },
        Rename { from: String },
    }
}
